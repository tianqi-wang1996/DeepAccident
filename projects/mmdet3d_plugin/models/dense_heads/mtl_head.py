...
class MultiTaskHead(BaseModule):
    ...
    def inference(self, predictions, img_metas, rescale):
        ...
        if self.task_enbale.get('motion', False):
            if len(predictions['motion']) > 1:
                res['motion_predictions'], res['motion_segmentation'], res['motion_instance'] = [], [], []
                for prediction in predictions['motion']:
                    seg_prediction, pred_consistent_instance_seg = self.task_decoders['motion'].inference(
                        prediction
                    )

                    res['motion_predictions'].append(prediction)
                    res['motion_segmentation'].append(seg_prediction)
                    res['motion_instance'].append(pred_consistent_instance_seg)
            else:
                seg_prediction, pred_consistent_instance_seg = self.task_decoders['motion'].inference(
                    predictions['motion'],
                )
