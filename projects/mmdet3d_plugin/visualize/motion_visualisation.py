...
from ..datasets.utils.instance import predict_instance_segmentation_and_trajectories, predict_instance_segmentation_and_trajectories_accident, predict_instance_segmentation_and_trajectories_accident_gt
...
def plot_instance_map(instance_image, instance_map, instance_colours=None, bg_image=None, overlay=False):
    ...
    if not overlay:
        ...
        for key, value in instance_colours.items():
            tint_factor = 0.5
            value = (value + (255 - value) * tint_factor).astype(np.int)
            plot_image[instance_image == key] = value
...
def visualise_output(labels, output):
    ...
    for t in range(sequence_length):
        ...
        # Ground truth
        if labels is not None:
            labels['segmentation'] = labels['segmentation'].to(torch.int)
        ...
        # Predictions
        if output is not None:
            output['segmentation'] = output['segmentation'].to(torch.int)
            unique_ids = torch.unique(
...
def plot_motion_label(motion_preds):
    consistent_instance_seg, matched_centers, _ = predict_instance_segmentation_and_trajectories(motion_preds,
    ...
    for instance_id in unique_ids:
        ...
        for t in range(len(path) - 1):
            ...
            cv2.line(trajectory_img, (round(path[t][0]), round(path[t][1])),
                     (round(path[t + 1][0]), round(path[t + 1][1])), color, 4)
    ...
    vis_image = plot_instance_map(
        consistent_instance_seg[0, -1].cpu().numpy(), instance_map, bg_image=vis_image, overlay=True)
...
def plot_motion_prediction(motion_preds):
    ...
    consistent_instance_seg, matched_centers, _ = predict_instance_segmentation_and_trajectories_accident(motion_preds,
                                                                                                 compute_matched_centers=True,
                                                                                                 make_consistent=True)
    ...
    trajectory_img = np.zeros(vis_image.shape, dtype=np.uint8)
    # final_mask = consistent_instance_seg[0, -1].cpu().numpy()

    last_seg = consistent_instance_seg[0, -1].cpu().numpy()

    # trajectory_img = np.ones(vis_image.shape, dtype=np.uint8) * 255
    for instance_id in unique_ids:
        path = matched_centers[instance_id]
        color = instance_colours[instance_id].tolist()
        for t in range(len(path) - 1):
            ...
            cv2.line(trajectory_img, (round(path[t][0]), round(path[t][1])),
                     (round(path[t + 1][0]), round(path[t + 1][1])), color, 4)
        mask = last_seg == instance_id
        trajectory_img[mask] = color

    temp_img = cv2.addWeighted(vis_image, 0.6, trajectory_img, 0.4, 1.0)
    mask = ~ np.all(trajectory_img == 0, axis=2)
    vis_image[mask] = temp_img[mask]

    return vis_image
    # pdb.set_trace()
    # trajectory_img[~mask] = 255
    # return trajectory_img


def plot_motion_prediction_with_map(motion_preds, map_img):
    ...
    mask = ~ np.all(vis_image == 255, axis=2)
    result_img = map_img.copy()
    result_img[mask] = vis_image[mask]
...
def plot_motion_label_with_map(motion_preds, map_img):
    # consistent_instance_seg, matched_centers, _ = predict_instance_segmentation_and_trajectories(motion_preds,
    #                                                                                              compute_matched_centers=True,
    #                                                                                              make_consistent=True)


    consistent_instance_seg, matched_centers, _ = predict_instance_segmentation_and_trajectories_accident_gt(motion_preds,
                                                                                                 compute_matched_centers=True,
                                                                                                 make_consistent=False)
