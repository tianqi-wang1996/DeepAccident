...
class FeatureWarper(object):
    ...
    def warp_features(self, x, flow, mode='nearest'):
        ...
        # np.where(x[0][0]>=0.85)
        # (array([ 99, 100, 100, 100, 101]), array([100,  99, 100, 101, 100]))
    ...
    def cumulative_warp_features(self, x, flow, mode='nearest', bev_transform=None):
        ...
        # returned feature order t - 1, t - 0.5, t
    ...
    def cumulative_warp_features_single(self, x, flow, mode='nearest', bev_transform=None):
        ...
        # flow direction: ego -> other, no need to revert since self.warp_features() eventually revert it

        cum_flow = pose_vec2mat(flow)

        # cum_flow = invert_pose_matrix(cum_flow)
        ...
        out = self.warp_features(x, warp_flow, mode)
    ...
    def cumulative_warp_features_v2x(self, x, flow, mode='nearest', bev_transform=None):
        ...
        for i in range(1, x.shape[1]):
            ...
            if bev_transform is not None:
                # points 先做 inverse_bev_transform，再做 motion 变换，再做 bev_transform
                # warp_flow = bev_transform @ cum_flow @ bev_transform.inverse()
                warp_flow = bev_transform[:, i] @ cum_flow @ bev_transform[:, i].inverse()
            else:
                warp_flow = cum_flow.clone()
