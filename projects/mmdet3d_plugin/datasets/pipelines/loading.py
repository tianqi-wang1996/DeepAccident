    ...
    def _load_bboxes_3d(self, results):
        ...
        for ann_info in results['ann_info']:
            if ann_info is not None:
                ...
                ego_bboxes_3d.append(ann_info['ego_bboxes_3d'])
            else:
                gt_bboxes_3d.append(None)
                ego_bboxes_3d.append(None)

        results['gt_bboxes_3d'] = gt_bboxes_3d
        results['bbox3d_fields'].append('gt_bboxes_3d')
        results['ego_bboxes_3d'] = ego_bboxes_3d
        # results['bbox3d_fields'].append('ego_bboxes_3d')
...
@PIPELINES.register_module()
class LoadMultiViewImageFromFiles_DeepAccident(LoadMultiViewImageFromFiles_MTL):
    """Load multi channel images from a list of separate channel files.
    ...
    def __init__(self, is_train=False, using_ego=False, temporal_consist=False,
        ...
        super().__init__(
            is_train=is_train, using_ego=using_ego, temporal_consist=temporal_consist,
            data_aug_conf=data_aug_conf,
            load_seg_gt=load_seg_gt, num_seg_classes=num_seg_classes, select_classes=select_classes
        )

    def get_img_inputs(self, results, specify_resize=None, specify_flip=None):
        img_infos = results['img_info']
        lidar2ego_mat = torch.Tensor(results['lidar_to_ego_matrix'].astype(np.float32))
        ...
        for frame_id, img_info in enumerate(img_infos):
            ...
            for cam in cams:
                ...
                intrin = torch.Tensor(cam_data['camera_intrinsic_matrix'])

                # extrinsics

                cam2lidar_mat = torch.Tensor(np.linalg.inv(cam_data['lidar_to_camera_matrix']).astype(np.float32))
                rot = torch.Tensor(cam2lidar_mat[:3, :3])
                tran = torch.Tensor(cam2lidar_mat[:3, 3])


                # to LiDAR coordinate
                if self.using_ego:
                    cam2ego = lidar2ego_mat @ cam2lidar_mat
...
@PIPELINES.register_module()
class LoadAnnotations3D_DeepAccident(LoadAnnotations3D_MTL):
    """Load Annotations3D.
