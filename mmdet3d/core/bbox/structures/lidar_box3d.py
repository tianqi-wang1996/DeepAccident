...
class LiDARInstance3DBoxes(BaseInstance3DBoxes):
    ...
    def corners_pitch(self, lidar_pitch_radian):
        """torch.Tensor: Coordinates of corners of all the boxes
        in shape (N, 8, 3).
        ...
        # import pdb
        # print(lidar_pitch_radian / 3.1415926 * 180)
        # if abs(lidar_pitch_radian/3.1415926 * 180) > 3:
        #     pdb.set_trace()
        rotation_matrix_pitch = torch.Tensor([
            [np.cos(lidar_pitch_radian), 0.0, np.sin(lidar_pitch_radian)],
            [0.0, 1.0, 0.0],
            [-np.sin(lidar_pitch_radian), 0.0, np.cos(lidar_pitch_radian)]],
        device=corners.device)
        new_corners = torch.ones(corners.shape, device=corners.device)
        for index in range(corners.shape[0]):
            new_corners[index] = (torch.matmul(rotation_matrix_pitch, corners[index].T)).T

        new_corners += self.tensor[:, :3].view(-1, 1, 3)
        # corners += self.tensor[:, :3].view(-1, 1, 3)
        return new_corners
